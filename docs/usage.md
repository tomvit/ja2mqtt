...
The process initially reads the configurations, establishes connections with the serial interface and MQTT broker by subscribing to the specified topics and using a client ID that uses a [prefix defined in the ja2mqtt definition file](configuration/ja2mqtt). Note that when multiple instances of ja2mqtt are running, the client ID must be unique unless your MQTT broker allows multiple connections from clients using the same client ID. If the client ID is not unique, the MQTT broker will disconnect the client with the same client ID that was connected first. 

```{note}
When you use Mosquitto MQTT broker, you can use the `allow_anonymous` option to allow multiple connections from clients using the same client ID. However, this option is not recommended for production environments.
```

The ja2mqtt run process then starts workers responsible for reading data from the serial interface and handling MQTT events. These workers perform operations such as sending events to MQTT or writing data to the serial interface based on the received data. Additionally, the process generates a PID file located in the user's home directory at `~/.ja2mqtt/ja2mqtt.pid`. This file serves the purpose of checking whether the same process is already running. It is also used by the `stop` command to send a termination signal to the running process.
